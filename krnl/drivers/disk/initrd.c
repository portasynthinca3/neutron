...
uint8_t initrd_init(void){
    ...
    for(int i = 0; i < handle_count; i++){
        ...
        if(EFI_ERROR(status)) {
            krnl_write_msgf(__FILE__, __LINE__, "UEFI simple filesystem protocol isn't supported");
            return 2;
        }
        //Try to find the file protocol from that
        status = sfs_prot->OpenVolume(sfs_prot, &root_file_prot);
        if(EFI_ERROR(status)) {
            krnl_write_msgf(__FILE__, __LINE__, "error opening volume");
            return 2;
        }
    ...
    initrd_rd_retry:
        ...
        if(status == EFI_BUFFER_TOO_SMALL){
            ...
            krnl_write_msgf(__FILE__, __LINE__, "info retrieve error");
            return 4;
        }
        //Get the initrd file
    ...
    krnl_write_msgf(__FILE__, __LINE__, "initrd file not found");
