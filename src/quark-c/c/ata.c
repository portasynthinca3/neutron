...
uint8_t ata_get_type(uint8_t bus, uint8_t device){
    ...
    ata_soft_reset(bus);
    //Select the device
    outb((bus ? ATA_SECO_DEVCT : ATA_PRIM_DEVCT) + ATA_REG_DEVSEL, 0xA0 | (device << 4));
    //Wait 400us
    ata_wait_100us(4);
    //Read the signature bytes
    uint8_t ch = inb((bus ? ATA_SECO_DEVCT : ATA_PRIM_DEVCT) + ATA_REG_CYL_HI);
    uint8_t cl = inb((bus ? ATA_SECO_DEVCT : ATA_PRIM_DEVCT) + ATA_REG_CYL_LO);
